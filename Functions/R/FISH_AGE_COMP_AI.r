...
LENGTH4AGE_BY_CATCH_AI<-function(fsh_sp_str=202 ,fsh_sp_label = "'PCOD'",sy=fsh_age_start_yr,ly=2021, STATE=1,max_age=10){
	require(RODBC)
	require(data.table)
	require(reshape2)
  require(rgdal)
...
## fill in the blanks code  Fill in years,trimester,area,gear with state port data if federal data are missing
  ...
  yy3<-yy3[,c(1:4,7)]
    ...
    age=data.table(GET_DOM_AGE(fsh_sp_str=202,sp_area="'AI'",max_age=10))
    age<-age[!is.na(AGE)]
